...
- **Stale deploy path — ignore `docker-compose.yml` and `deploy/`:** this is a half-finished Docker deployment scaffold from before real user auth existed. Its `APP_USERNAME`/`APP_PASSWORD` env vars do nothing in the current app (auth is the `User` table + JWT session, per Auth helpers below). The Electron installer + native PostgreSQL path documented above is the current, working deploy path. `deploy/backup.sh` also predates the native install and needs adjusting before use (see Phase 7 of `docs/launch-checklist.md`).
