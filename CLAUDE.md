...
# Field-tech LAN access (Base): open the OS firewall on the office machine. Required
...
- **Installer output — Windows:** `dist-electron/WhiteVanOps-Base-Setup.exe` / `WhiteVanOps-Plus-Setup.exe` (customer installers) and `WhiteVanOps-Base-Trial-Setup.exe` / `WhiteVanOps-Plus-Trial-Setup.exe` (30-day demo builds) — NSIS, creates desktop shortcut + Start Menu entry automatically. **macOS:** the same four products as `.dmg`, each built in both `arm64` and `x64` — `WhiteVanOps-Base-Setup-arm64.dmg` / `-x64.dmg`, `WhiteVanOps-Plus-Setup-arm64.dmg` / `-x64.dmg`, and the trial equivalents (8 files total across both plans/variants). Every artifact bundles portable PostgreSQL (see `electron/postgres.js` above) — fully self-contained on machines with no existing database.
