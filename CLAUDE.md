...
# One installer per platform — the 30-day trial is chosen on first launch, not built separately.
...
- `electron/postgres.js` — bundled-PostgreSQL lifecycle, shared between platforms. The installer ships portable PostgreSQL binaries from the project's `pgsql/` (Windows, ~133 MB, gitignored — see `MANUAL_Setup_Installation.md` §1 to recreate) or `pgsql-mac/` (macOS, ~160 MB, gitignored, sourced from Postgres.app — see the same section) — both are copied by `package.json`'s per-platform `extraResources` to the **same** `resources/pgsql` destination, so the rest of the code never needs to know which platform's binaries it's running. On launch it: generates `<appData>/whitevanops/.env.local` with unique random credentials if missing (see the Contents/Resources rule above); skips entirely when the `DATABASE_URL` port already has a listener (office PM2 machine) or the host isn't localhost; **throws** (surfaced by `main.js` as a startup-error dialog) when the URL is localhost, the port is free, and the bundled binaries are missing — booting the web server anyway would just 500 every query ("Failed to load dashboard data", 2026-07-12); otherwise initdb's `<appData>/whitevanops/pgdata` on first run, starts PG via `pg_ctl` (127.0.0.1 only), applies migrations, and bootstraps admin/admin. A `bootstrap-complete` sentinel gates first-run; failures wipe pgdata and retry next launch. **Gotcha:** `pg_ctl start`/`stop` must run with `stdio: 'ignore'` — the postgres daemon inherits piped stdio and `execFileSync` hangs forever.
- **Migrations apply on every launch, not just first run** (fixed 2026-09-23 — before this, `schema.sql` ran only on first launch, so every in-place upgrade 500'd on the first query touching a new column). `electron/migrate.js`'s `runMigrations()` is called by `postgres.js` on first run, on every later managed start, and on the "already has our database running" reuse path. It never runs against an external or PM2 database; those use `npx prisma migrate deploy`. Rules:
  - **Tracking is Prisma's own `_prisma_migrations` table**, written in Prisma's row format with Prisma's checksum (sha256 of `migration.sql`), so `prisma migrate status`/`deploy` agree with the runner (verified). **Never edit a migration that has shipped**; add a new one.
  - **A new migration needs nothing extra.** `scripts/electron-build.js` step 5 bundles every folder into `resources/db/migrations.json`, and step 7d asserts the packaged count. Each migration runs in its own transaction, so don't write one that can't (`CREATE INDEX CONCURRENTLY`, `ALTER TYPE … ADD VALUE`).
  - **`LEGACY_PROBES` is frozen** at the 13 migrations that predate tracking. It only identifies installs made from the old `schema.sql`, taking the longest prefix of passing probes as already applied. Don't add probes for new migrations.
  - A non-empty database is `pg_dump`'ed to `<appData>/whitevanops/pre-upgrade-backups/` (newest 3 kept) before any change. A failed dump or failed migration throws into `main.js`'s startup-error dialog, and `postgres.js` stops the Postgres it started so it doesn't outlive the error.
...
- **Installer output — Windows:** `dist-electron/WhiteVanOps-Setup.exe` — NSIS, creates desktop shortcut + Start Menu entry automatically. **macOS:** `WhiteVanOps-Setup-arm64.dmg` / `-x64.dmg`. There is exactly one installer per platform — no Base/Plus split (see "License / activation" below) and no separate trial build (see "30-day trial" below). A build meant for public download must be made **without** a project `.env.local`, so each install generates its own secrets. Every artifact bundles portable PostgreSQL (see `electron/postgres.js` above) — fully self-contained on machines with no existing database.
...
- **There is one installer per platform, not two.** `WhiteVanOps-Setup.exe` (Windows) /
  ...
  has a plan axis to assert (and, since 2026-09-24, no trial axis either).
...
### 30-day trial (one installer; chosen on first launch — 2026-09-24)

There is no trial build any more. The one installer's first-launch activation window
(`electron/activation.html`, `mode=first`) offers **Activate** (a `WVO-` key, checked against
Firestore as always), **Start 30-day free trial**, or client mode. The build-time `WVO_IS_TRIAL`
flag, `isTrialBuild()`, `electron:build:trial` and the `-Trial-Setup` artifacts are all gone;
`electron-build.js` strips any stale `WVO_IS_TRIAL` line from a bundled `.env.local`.

- **Who is allowed to run is decided in `electron/main.js` before the window opens** (`getAccess()`): licensed (a verified `license.json`, or the offline `trial-unlock.json`) → open; trial with days left → open; trial expired → the activation window in `mode=expired` (key only, closing quits); nothing yet → `mode=first`. A `trial.json` that exists but fails verification counts as **expired** (fail closed, same as the server).
- **Trial state lives in two places; the earliest wins.** `electron/trial.js` writes the signed `trial.json` when the trial starts, and `reconcileTrialWithDb()` (after the database is up) mirrors it into `SystemSetting` key `trial_anchor`. Deleting the file doesn't reset the clock — it's rewritten from the database, and the reset lands in `mode=expired`. Deleting both means deleting the customer's data. A reconcile failure never blocks startup.
- **Signatures are duplicated in plain JS** (`electron/trial.js` `signTrialAnchor`/`signTrialUnlock`) and must stay byte-identical to `src/lib/licenseCrypto.ts`. `electron/trial.test.js` asserts parity; if it fails, every trial reads as tampered and locks on day one.
- **The server never starts a trial.** `src/lib/trial.ts` `getTrialStatus()` only reads `trial.json`: no file → not a trial, not locked (office PM2 box, dev). Activation always wins: a verified `license.json` or `trial-unlock.json` → `isTrial: false`, however old `trial.json` is. The "is this machine activated?" readers (`readVerifiedBaseLicense`, `readVerifiedTrialUnlock`) live in the leaf `licenseCrypto.ts` so `license.ts` and `trial.ts` don't import each other.
- **Server-side enforcement is a backstop** for phones and long-lived sessions (7 days):
  - `trialLocked` is stamped into the JWT at login, and the middleware sends locked sessions to `/trial-expired`. Locked sessions may reach only `/trial-expired`, `/api/license` and `/api/auth/logout`. They are let through **before** role routing, and `/trial-expired` is in `TECH_ALLOWED_PREFIXES`; without both, a tech redirect-loops (bug found and fixed 2026-09-24).
  - `GET /api/dashboard` and `GET /api/field` return `403 { code: "trial_expired" }` via `trialExpiredResponse()` (`src/lib/trialGuard.ts`), and the clients redirect on it (`isTrialExpiredResponse()` in the browser-safe `src/lib/trialExpired.ts`).
  - **Writes are deliberately not blocked**: a tech's queued field work must never be rejected into "needs attention".
- **An office PC left open past day 30:** `watchTrialExpiry()` re-checks hourly and shows `mode=expired` over the app. **Help → Enter activation key…** (`mode=activate`, closable) converts early.
- **Conversion is a normal `WVO-` key.** The per-machine `license-manager.js --unlock-trial` JSON code remains only as an offline fallback: Settings → License & Plan, or the trial-ended page, under "No internet at the office?"; superuser-only `POST /api/license` `unlock-trial`.
- History: `docs/superpowers/specs/2026-07-13-trial-demo-installer-design.md` describes the retired separate-trial-build design. Customer-facing steps: `MANUAL_Setup_Installation.md` §6.
...
`.github/workflows/ci.yml` runs on every push (not just PRs). Its `check` job (`ubuntu-latest`) does the full pass: install, `prisma generate`, `tsc`, lint (non-blocking), `npm test`, `npm run build`, and a conditional `storefront/` type check. A separate `check-macos` job (`macos-latest`, added alongside macOS Electron support) runs only `tsc` + `npm test` — deliberately narrower, since it exists to catch a shared-code-path regression that only breaks on macOS (a `path.join` assumption, an unguarded `process.platform` branch), not to build or package anything.

`.github/workflows/build-installers.yml` (added 2026-09-24) **builds the real installers** on GitHub's runners — manually (Actions → "Build installers" → Run workflow) or on a `v*` tag push:
- The `windows` job downloads the EDB PostgreSQL zip into `pgsql/`, runs `npm run electron:build`, and uploads `WhiteVanOps-Setup.exe` plus its buildinfo.
- The `macos` job stages `pgsql-mac/` with `scripts/ci/stage-pgsql-mac.sh` (the §1 manual steps automated: latest Postgres.app PG 17 dmg, the four binaries, an `otool`-computed dylib closure, `plpgsql`/`dict_snowball`, `share/postgresql`, then an initdb/start/pg_dump/stop self-test). It then runs `npm run electron:build:mac` and uploads both `.dmg`s.
- Both are **generic builds** with no `.env.local`, so each install generates its own secrets. A per-customer build with a bundled `.env.local` is still made by hand.
