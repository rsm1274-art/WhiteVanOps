...
| 30-day trial | Complete — **no separate trial installer since 2026-09-24**: the one installer offers "Start 30-day free trial" or an activation key on first launch. Signed, machine-bound start date kept in `trial.json` and mirrored into the database (earliest wins); after day 30 the app won't open without a key, data untouched. See `HANDOFF_2026-09-24-single-installer-trial.md` |
