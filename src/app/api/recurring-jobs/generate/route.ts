...
// Blocking conflicts (open repair, time-off, equipment already booked) skip
// that occurrence individually and are reported back — never silently
// dropped, never blocking the rest of the batch. A same-day tech/van
// double-booking is advisory: the job is still created and the date is
// reported under `warnings` so the office can review it.
export async function POST(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < BATCH_SIZE; i++) {
      ...
      if (conflict.error) {
        skipped.push({ date: occurrenceDate.toISOString(), reason: conflict.error });
      } else {
        for (const reason of conflict.warnings) {
          warnings.push({ date: occurrenceDate.toISOString(), reason });
        }
