...
describe("signTrialUnlock / verifyTrialUnlock", () => {
  it("verifies a correctly signed payload (round-trip)", () => {
    const sig = signTrialUnlock("test-machine-id", null);
    const payload = { machineId: "test-machine-id", expiresAt: null, notes: null, sig };
    expect(verifyTrialUnlock(payload, "test-machine-id")).toBe(true);
  });

  it("verifies a payload with a non-null expiresAt", () => {
    const expiresAt = "2027-01-01T00:00:00.000Z";
    const sig = signTrialUnlock("test-machine-id", expiresAt);
    const payload = { machineId: "test-machine-id", expiresAt, notes: "note", sig };
    expect(verifyTrialUnlock(payload, "test-machine-id")).toBe(true);
  });

  it("rejects a payload whose machineId does not match the caller's machine", () => {
    const sig = signTrialUnlock("other-machine", null);
    const payload = { machineId: "other-machine", expiresAt: null, notes: null, sig };
    expect(verifyTrialUnlock(payload, "test-machine-id")).toBe(false);
  });

  it("rejects a tampered signature", () => {
    const payload = { machineId: "test-machine-id", expiresAt: null, notes: null, sig: "bogus" };
    expect(verifyTrialUnlock(payload, "test-machine-id")).toBe(false);
  });
